...
def remove_ceiling_floor(in_pcl, z_offset=10):
  out_pcl = in_pcl[np.abs(in_pcl[:, 2]) < z_offset, :]
  return out_pcl
  ...
  for range_idx in range(params['num_range_bins'] - args.min_range):
    for az_idx in range(params['num_azimuth_bins']):
      for el_idx in range(params['num_elevation_bins']):
        ...
        pcl[el_idx,az_idx,range_idx,5:] = np.array([range_idx + args.min_range, az_idx, el_idx])
  print("Pcl size: {}".format(pcl.shape))
  pcl = pcl.reshape(-1,8)
  return pcl
...
def animate_plot(i):
  ...
  if plot_data[i][3] == 'lidar':
    ...
    lidar_pc = remove_ceiling_floor(lidar_pc)
  ...
  # if not plot_data[i][3] == 'lidar':
  #   plots['radar'].remove()
  #   norm_val = np.linalg.norm(radar_pc, axis=1)
  #   plots['radar'] = ax.quiver(radar_pc[:,0],
  #                              radar_pc[:,1],
  #                              radar_pc[:,2],
  #                             radar_pc[:,0] / norm_val * radar_pc_local[:, 4],
  #                             radar_pc[:,1] / norm_val * radar_pc_local[:, 4],
  #                             radar_pc[:,2] / norm_val * radar_pc_local[:, 4],
  #                             color='b', label=radar_label)

  if not plot_data[i][3] == 'radar':
    radar_pc[:,3] -= radar_pc[:,3].min()
    radar_pc[:,3] /= radar_pc[:,3].max()

    plots['radar'].remove()
    viridis = cm.get_cmap('viridis', 12)
    colors = viridis(radar_pc[:,3])
    print("Max: {}, Min: {}".format(np.max(radar_pc[:, 3]), np.min(radar_pc[:, 3])))
    colors[:, 3] = radar_pc[:, 3]
    colors[:, :3] = 0
...
if __name__ == '__main__':
  ...
  # k = np.ones_like(radar_pc[:,0])
  # plots['radar'] = ax.quiver(radar_pc[:,0],
  #                             radar_pc[:,1],
  #                             radar_pc[:,2],
  #                             k,k,k, color='b', label=radar_label)
