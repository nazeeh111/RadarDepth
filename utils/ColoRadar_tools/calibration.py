...
def apply_phase_calibration(adc_samples, phase_calib):
  ...
  phase_cal_mat = phase_calib['cal_matlix']
...
def apply_frequency_calibration(adc_samples, freq_calib, wave_config):
  ...
  num_range_bins = wave_config['num_adc_samples_per_chirp']
  ...
  freq_calib /= num_range_bins
  freq_calib_vec = np.expand_dims(freq_calib, -1) * np.arange(num_range_bins)
  ...
  return freq_corrected_adc_samples
